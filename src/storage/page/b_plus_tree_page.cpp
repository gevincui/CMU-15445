...
auto BPlusTreePage::GetMinSize() const -> int {
  // min size 表示数据页能存储的key的最小个数，即再小于它就要发生merge

  // 如果叶子数据页：
  // 实际能存储的最大size为max_size_ - 1，减1是为了因为方便做merge
  // 除2向上取整即为min_size，即(max_size - 1 + 1) / 2
  if(IsLeafPage()) {
    return (max_size_ -1 + 1) / 2;
  }

  // 如果是非叶子数据页：
  // 实际能存储的最大size为max_size_ - 2, 第一个减1是为了因为方便做merge，第二个减1是因为非叶子数据页第一个key是废弃的
  // 除2向上取整加一即为min_size，，即(max_size - 2 + 1) / 2 + 1，加1是因为要把第一个废弃的key加回来
